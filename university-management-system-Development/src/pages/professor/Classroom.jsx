...
export default function ProfessorClassroom() {
  ...
  // Prevent app from crashing if data context collections aren't loaded yet
    ...
    return (
      <Card>
        <EmptyState 
          ...
          action={<Button onClick={() => navigate('/professor/courses')}>Back to courses</Button>} 
        />
      </Card>
    )
  }
...
function MaterialsTab({ course }) {
  ...
  const items = useMemo(() => {
    return materials.filter((m) => m?.courseId === course?.id)
  }, [materials, course])
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <Card>
          <EmptyState 
            ...
            message="Upload lecture slides, PDFs, or links for your students." 
            action={<Button icon={Plus} onClick={() => setOpen(true)}>Upload material</Button>} 
          />
        </Card>
      ) : (
      ...
      <ConfirmDialog 
        open={!!del} 
        onClose={() => setDel(null)} 
        onConfirm={() => { deleteMaterial(del?.id, currentUser?.id); toast('Material deleted.', 'info') }}
...
function AnnouncementsTab({ course, author }) {
  ...
  const items = useMemo(() => {
    return announcements
      .filter((a) => a?.scope === 'course' && a?.courseId === course?.id)
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
                      ...
                      <Badge tone="emerald">{gradedCount} graded</Badge>
                    </div>
                  </div>
...
function AttendanceTab({ course, roster }) {
  ...
  const records = useMemo(() => {
    return attendance.filter((a) => a?.courseId === course?.id).sort((a, b) => new Date(b?.date) - new Date(a?.date))
  }, [attendance, course])
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
              ...
              <th className="px-5 py-3">New score</th>
