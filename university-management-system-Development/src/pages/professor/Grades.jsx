...
export default function ProfessorGrades() {
  ...
  // Automatically update active course selection once list loads
  useEffect(() => {
    if (myCourses.length > 0 && !courseId) {
  ...
  const saveAll = () => {
    ...
    if (n) { 
      toast(`${n} grade${n > 1 ? 's' : ''} saved.`, 'success')
      setScores({}) 
    } else {
      toast('No scores entered.', 'info')
    }
  }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                  ...
                  <th className="px-5 py-3">New score (0–100)</th>
                    ...
                    <tr key={s?.id} className="hover:bg-slate-50/60">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-slate-700">{fullName(s)}</p>
                            <p className="text-xs text-slate-400">{s?.studentId || s?.id}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3">
                        <input 
                          ...
                          className="w-24 rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/30" 
                          placeholder="0–100" 
                        />
                      </td>
                      <td className="px-5 py-3">
                        {preview ? (
                          <span className={classNames('font-bold', gradeColor(preview))}>{preview}</span>
                        ) : (
                          <span className="text-slate-300">—</span>
                        )}
                      </td>
                      <td className="px-5 py-3 text-right">
                        <Button size="sm" icon={Save} onClick={() => save(s?.id)}>Save</Button>
                      </td>
                    </tr>
                  )
                })}
