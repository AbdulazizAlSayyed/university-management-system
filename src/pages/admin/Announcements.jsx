...
import { Megaphone, Plus, Trash2, Pin, Globe, AlertTriangle } from 'lucide-react'
import * as adminApi from '../../api/admin'
import { useToast } from '../../context/ToastContext'
import { PageHeader, Card, Button, IconButton, Badge, Modal, FormField, Input, Textarea, EmptyState, LoadingState } from '../../components/ui'
...
export default function AdminAnnouncements() {
  ...
  const load = useCallback(async () => {
    ...
    try { setItems(await adminApi.listAnnouncements()) } catch (e) { setError(adminApi.errMsg(e)) } finally { setLoading(false) }
  }, [])
  useEffect(() => { load() }, [load])
  ...
  return (
    <div>
      <PageHeader title="System Announcements" subtitle="Post announcements visible to all users." icon={Megaphone}
        actions={<Button icon={Plus} onClick={() => setModalOpen(true)}>New announcement</Button>} />
      {loading ? <LoadingState label="Loading announcements..." /> : error ? (
        <Card><EmptyState icon={AlertTriangle} title="Could not load announcements" message={error} action={<Button onClick={load}>Retry</Button>} /></Card>
      ) : items.length === 0 ? (
          ...
          {items.map((a) => (
            <Card key={a.id} className="p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <IconButton icon={Trash2} title="Delete" onClick={() => remove(a.id)} className="hover:text-red-600" />
              </div>
            </Card>
          ))}
        </div>
