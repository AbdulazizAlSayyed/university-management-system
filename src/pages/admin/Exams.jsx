...
import { CalendarClock, Plus, Pencil, Trash2, MapPin, Clock, AlertTriangle } from 'lucide-react'
import * as adminApi from '../../api/admin'
import { useToast } from '../../context/ToastContext'
import {
  PageHeader, Card, Button, IconButton, Badge, Modal, ConfirmDialog, FormField,
  Input, Select, EmptyState, LoadingState,
} from '../../components/ui'
...
export default function AdminExams() {
  ...
  const load = useCallback(async () => {
    ...
    try { const [x, c] = await Promise.all([adminApi.listExams(), adminApi.listCourses()]); setExams(x); setCourses(c) }
    catch (e) { setError(adminApi.errMsg(e)) } finally { setLoading(false) }
  }, [])
  useEffect(() => { load() }, [load])
  ...
  return (
    <div>
      <PageHeader title="Exam Scheduling" subtitle="Create and manage the examination timetable." icon={CalendarClock}
        actions={<Button icon={Plus} onClick={openCreate}>Schedule exam</Button>} />
      {loading ? <LoadingState label="Loading exams..." /> : error ? (
        <Card><EmptyState icon={AlertTriangle} title="Could not load exams" message={error} action={<Button onClick={load}>Retry</Button>} /></Card>
      ) : (
        <Card className="overflow-hidden">
          {exams.length === 0 ? (
            <EmptyState icon={CalendarClock} title="No exams scheduled" message="Schedule your first exam." action={<Button icon={Plus} onClick={openCreate}>Schedule exam</Button>} />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b border-slate-100 bg-slate-50 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                  <th className="px-5 py-3">Exam</th><th className="px-5 py-3">Course</th><th className="px-5 py-3">Type</th>
                  <th className="px-5 py-3">Date</th><th className="px-5 py-3">Time</th><th className="px-5 py-3">Room</th><th className="px-5 py-3 text-right">Actions</th>
                </tr></thead>
      ...
      <ConfirmDialog open={!!confirmDel} onClose={() => setConfirmDel(null)} onConfirm={doDelete}
        title="Delete exam?" message={confirmDel ? `Remove "${confirmDel.title}" from the timetable?` : ''} confirmLabel="Delete" />
    </div>
  )
}
