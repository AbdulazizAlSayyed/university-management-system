...
export default function ProfessorAttendance() {
  ...
  const rate = (rec) => {
    ...
    return Math.round((rec.records.filter((r) => r?.present).length / rec.records.length) * 100)
  }
  ...
  const summary = useMemo(() => {
    return roster.map((s) => {
      ...
      return { 
        ...
        total: totalCount, 
        pct: totalCount ? Math.round((presentCount / totalCount) * 100) : null 
      }
    })
  }, [roster, records])
        ...
        <Card>
          ...
          {records.length === 0 ? (
            <EmptyState 
              icon={CalendarCheck} 
              title="No sessions" 
              message="Take attendance to start tracking." 
              action={roster.length > 0 && <Button size="sm" icon={Plus} onClick={startSession}>Take attendance</Button>} 
            />
          ) : (
                ...
                <li key={rec?.id} className="flex items-center justify-between px-5 py-3.5">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {formatDate(rec?.date, { weekday: 'short', month: 'short', day: 'numeric' })} · {rec?.records?.length || 0} students
                    </p>
                  </div>
