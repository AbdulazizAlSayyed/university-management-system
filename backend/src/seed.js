...
async function seed() {
  ...
  await Promise.all([
    ...
    Exam.deleteMany({}), Announcement.deleteMany({}), CalendarEvent.deleteMany({}), AuditLog.deleteMany({}),
  ])
  ...
  const coursesData = [
    ...
    { code: 'CS310', name: 'Full-Stack Web Development', description: 'Building web apps with the MERN stack.', credits: 3, capacity: 30, professorId: profSmith._id, schedule: 'Mon & Wed - 14:00-15:30', room: 'Lab C-12', color: 'bg-indigo-500' },
  ...
  await enroll(students[0], byCode.CS101)
  await enroll(students[0], byCode.CS201)
  await enroll(students[0], byCode.CS310)
  await enroll(students[1], byCode.CS101)
  await enroll(students[1], byCode.CS310)
  await enroll(students[2], byCode.CS101)
